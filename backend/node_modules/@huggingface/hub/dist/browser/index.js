...
async function xetWriteToken(params) {
  ...
  const promise = (async () => {
    const resp = await (_nullishCoalesce(params.fetch, () => ( fetch)))(
      `${params.hubUrl}/api/${params.repo.type}s/${params.repo.name}/xet-write-token/${params.rev}` + (params.isPullRequest ? "?create_pr=1" : ""),
      {
...
async function spaceInfo(params) {
  ...
  const search = new URLSearchParams([
    ...
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _218 => _218.additionalFields, 'optionalAccess', _219 => _219.map, 'call', _220 => _220((val) => ["expand", val])]), () => ( []))
  ]).toString();
  const response = await (params.fetch || fetch)(
    `${_optionalChain([params, 'optionalAccess', _221 => _221.hubUrl]) || HUB_URL}/api/spaces/${params.name}/revision/${encodeURIComponent(
      _nullishCoalesce(params.revision, () => ( "HEAD"))
    )}?${search.toString()}`,
    {
      headers: {
  ...
  const data = await response.json();
  return {
    ..._optionalChain([params, 'optionalAccess', _222 => _222.additionalFields]) && pick(data, params.additionalFields),
    id: data._id,
    name: data.id,
    sdk: data.sdk,
    likes: data.likes,
...
async function* uploadFilesWithProgress(params) {
  return yield* commitIter({
    ...
    fetch: async (input, init) => {
      ...
      _optionalChain([init, 'access', _223 => _223.signal, 'optionalAccess', _224 => _224.throwIfAborted, 'call', _225 => _225()]);
      xhr.send(init.body);
      return new Promise((resolve2, reject) => {
        ...
        if (init.signal) {
          init.signal.addEventListener("abort", () => {
            xhr.abort();
            try {
              _optionalChain([init, 'access', _226 => _226.signal, 'optionalAccess', _227 => _227.throwIfAborted, 'call', _228 => _228()]);
            } catch (err) {
              reject(err);
            }
          });
...
async function whoAmI(params) {
  ...
  if (typeof _optionalChain([response, 'access', _229 => _229.auth, 'access', _230 => _230.accessToken, 'optionalAccess', _231 => _231.createdAt]) === "string") {
